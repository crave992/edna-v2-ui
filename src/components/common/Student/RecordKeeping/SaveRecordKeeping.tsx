...
const SaveRecordKeeping: NextPage<SaveRecordKeepingProps> = ({ studentId, classId, lessonId, isLessonAssigned, recordKeeping,}) => {
  ...
  const plannedDate = recordKeeping?.plannedDate ? new Date(unitOfService.DateTimeService.convertToLocalDate(recordKeeping?.plannedDate)) : new Date();
  const presentedDate = recordKeeping?.presentedDate ? new Date(unitOfService.DateTimeService.convertToLocalDate(recordKeeping?.presentedDate)) : new Date();
  const practicingDate = recordKeeping?.practicingDate ? new Date(unitOfService.DateTimeService.convertToLocalDate(recordKeeping?.practicingDate)) : new Date();
  const acquiredDate = recordKeeping?.acquiredDate ? new Date(unitOfService.DateTimeService.convertToLocalDate(recordKeeping?.acquiredDate)) : new Date();

  const {formState: { errors }, handleSubmit, register, setValue, getValues, control,} = useForm<RecordKeepingModel>({
    resolver: yupResolver(RecordKeepingValidationSchema),
    defaultValues: {
      count: 0,
      status: recordKeeping?.status || "",
    },
  });

  const [action, setAction] = useState<string>(recordKeeping?.status || "");

  const [acquiredCount, setAcquiredCount] = useState<number>(recordKeeping?.acquiredCount || 0);
  ...
  const [practicingCount, setPracticingCount] = useState<number>(recordKeeping?.practiceCount || 0);
  ...
  const handleChange = (selectedOption: Option | null) => {
    ...
    if (selectedValue !== getValues().status) {
      setValue("status", selectedValue);
      if(selectedValue === "practicing"){
        setValue("count", practicingCount);
      }else if(selectedValue === "acquired"){
        setValue("count", acquiredCount);
      }
      submitData(getValues());
    }
  };
  ...
  const submitData = async (formData: RecordKeepingModel) => {
    ...
    const response = await unitOfService.RecordKeepingService.save(studentId, classId, lessonId, formData);
  ...
  return (
    <>
      {isLessonAssigned === false && form === false ? (
        <span className="btn_main size_small anchor-span" onClick={() => assignStudentToLesson(classId, studentId, lessonId)}>
          <FontAwesomeIcon icon={faPlus} /> Add
        </span>
      ) : isLessonAssigned === true && form === false ? (
        <span className="btn_main size_small anchor-span" onClick={() => setForm(true)}>
          ...
          <div className="lesson_state">
            <Select
              defaultValue={options.filter(
                (e) => e.value === recordKeeping?.status
              )}
              options={options}
              components={{
                Option: customOption,
...
export default SaveRecordKeeping;
