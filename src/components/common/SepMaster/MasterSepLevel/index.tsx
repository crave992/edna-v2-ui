...
import AddMasterSepLevel from "./AddMasterSepLevel";
import BulkUploadMasterSepLevel from "./BulkUploadMasterSepLevel";

const initialPageState: InPageState<SepLevelListResponseDto> = {
  id: 0,
  currentPage: 1,
  showLoader: false,
...
const MasterSepLevelPage: NextPage<SepLevelListParams> = (props) => {
  ...
  const deleteSepLevel = async () => {
    dispatch({
      type: InPageActionType.SHOW_LOADER,
      payload: true,
    });

    const response = await unitOfService.SepLevelService.delete(states.id);
    ...
    if (response && response.status === 204) {
      ...
      toast.success("SEP Level deleted successfully");
  ...
  const submitData = async (formData: SepLevelListParams) => {
    dispatch({
      type: InPageActionType.SHOW_LOADER,
      payload: true,
    });
  ...
  async function actionFunction(p: SepLevelListParams) {
    const qs = require("qs");
    await fetchSepLevel(p);
    router.push(
      {
  ...
  const fetchSepLevel = async (p?: SepLevelListParams) => {
    if (!p) {
      p = props;
    }
    const response = await unitOfService.SepLevelService.getAll(p);
  ...
  const formSelectChange = async (e: ChangeEvent<HTMLSelectElement>) => {
    if (e.target.name === "levelId") {
      ...
      setValue("sepAreaId", +e.target.value);
    ...
    <>
            ...
            <Col md={12} lg={12}>
              ...
              <div className="tableListItems">
                ...
                <Table striped hover className="custom_design_table mb-0">
                  ...
                  <tbody>
                    {states.data &&
                      states.data?.sepLevels?.map((sepLevel) => {
                        return (
                          <tr key={sepLevel.id}>
                            <td>{sepLevel.level.name}</td>
                            <td>{sepLevel.sepArea.name}</td>
                            <td>{sepLevel.name}</td>
                            <td className="text-center">
                              ...
                              >
                                <span
                                  ...
                                  onClick={() => openAddUpdateModal(sepLevel.id)}
                                >
                                  <FontAwesomeIcon
                                    icon={faPenToSquare}
                                    size="1x"
      ...
      {states.showDeleteModal && (
        <ConfirmBox
          ...
          onSubmit={deleteSepLevel}
          bodyText="Are you sure want to delete this sep level?"
          noButtonText="Cancel"
...
export default MasterSepLevelPage;
