...
interface HolidaysProps extends CommonProps {

}

const initialPageState: InPageState<HolidayDto[]> = {
    id: 0,
    currentPage: 1,
    showLoader: false,
...
const Holidays: NextPage<HolidaysProps> = (props) => {
    const unitOfService = container.get<IUnitOfService>(TYPES.IUnitOfService);
    const [states, dispatch] = useReducer(reducer<HolidayDto[]>, initialPageState);
    ...
    return (
        <>
            <div className='db_heading_block'>
                <h3 className="db_heading">Holidays</h3>
                <Button className="btn_main size_small" onClick={() => openAddUpdateModal(0)}>Add Holiday</Button>
            </div>
            <div className="table-responsive">
                <Table striped hover className="custom_design_table mb-0">
                    ...
                    <tbody>
                        {states.data &&
                            states.data?.map((holiday) => {
                                return (
                                    <tr key={holiday.id}>
                                        <td>{holiday.name}</td>
                                         {/* <td>{holiday.holidayType.name}</td> */}
                                        <td>{unitOfService.DateTimeService.convertToLocalDate(holiday.startDate)} - {unitOfService.DateTimeService.convertToLocalDate(holiday.endDate)}</td>
                                        <td className="text-center">
                                            ...
                                            >
                                                <span
                                                    ...
                                                    onClick={() => openAddUpdateModal(holiday.id)}
                                                >
                                                    <FontAwesomeIcon
                                                        icon={faPenToSquare}
                                                        size="1x"
