...
const AddLesson: NextPage<AddLessonProps> = (props) => {
    ...
    const fetchLesson = async (id: number) => {
        ...
        if (response && response.status === 200 && response.data.data) {
            ...
            setValue("sequenceNumber", lesson.sequenceNumber);
            setValue("sequentialAssignment", lesson.sequentialAssignment.toLowerCase());
            setValue("areaId", lesson.areaId);
            setValue("levelId", lesson.levelId);
            setValue("topicId", lesson.topicId);
            fetchAreaByLevel(lesson.levelId);
            fetchTopicByLevelAndArea(lesson.levelId, lesson.areaId);
        }
    };
