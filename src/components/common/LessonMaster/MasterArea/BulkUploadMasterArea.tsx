...
const BulkUploadMasterArea: NextPage<AddMasterAreaProps> = (props) => {
    const unitOfService = container.get<IUnitOfService>(TYPES.IUnitOfService);
    const [states, dispatch] = useReducer(modalReducer, initialState);

    const { formState, handleSubmit, register, setValue, control, getValues } = useForm<AreaModelBulkUpload>({
        resolver: yupResolver(AreaBulkValidationSchema),
        defaultValues: {
            levelId: 0
        },
    });
    ...
    const processFile = async (
        levelId: number,
        files: FileList
    ): Promise<{ status: boolean; data: AreaModel[]; message: string }> => {
                        ...
                        if (worksheet['!ref']) {
                            ...
                            if (rangeRowCount === 0 || rangeColumnCount !== headerValues.length) {
                                ...
                                resolve({
                                    status: true,
                                    message: '',
                                    data: dataArray as unknown as AreaModel[],
                                });
                            }
                        } else {
                            resolve({
    ...
    const submitData = async (model: AreaModelBulkUpload) => {
        ...
        if (fileResponse) {
            if (fileResponse.status) {
                ...
                response = await unitOfService.AreaService.postMultipleArea(fileResponse.data);
                ...
                if (response && (response.status === 201)) {
                    toast.success("Areas uploaded successfully");
                    dispatch({
    ...
    const downloadData = async () => {
        ...
        let downloadableData: LevelDto[] = [];
        const response = await unitOfService.LevelService.getAll();
        if (response && response.status === 200 && response.data.data) {
            downloadableData = response.data.data;
        }
        dispatch({
            type: InModalActionType.SHOW_LOADER,
            payload: false,
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Upload Areas');
        const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
        const excelData = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        saveAs(excelData, 'areas-upload.xlsx');
    };

    const transformData = (data: LevelDto[]) => {
...
export default BulkUploadMasterArea;
