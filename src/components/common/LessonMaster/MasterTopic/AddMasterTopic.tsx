...
interface AddMasterTopicProps extends CommonProps {    
    id: number;
    isOpen: boolean;
    onClose: (refreshRequired: boolean) => void;
}

const AddMasterTopic: NextPage<AddMasterTopicProps> = (props) => {
    ...
    const fetchTopic = async (id: number) => {
        ...
        if (response && response.status === 200 && response.data.data) {
            ...
            setValue("areaId", topic.area.id);
            setValue("levelId", topic.level.id);
            fetchAreaByLevel(topic.level.id);
        }
    };    
