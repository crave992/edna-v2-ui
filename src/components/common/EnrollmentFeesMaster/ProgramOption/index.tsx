...
import { ProgramOptionResponseDto } from "@/dtos/ProgramOptionDto";
import AddProgramOption from "./AddProgramOption";
import LevelListParams from "@/params/LevelListParams";

const initialPageState: InPageState<ProgramOptionResponseDto> = {
    id: 0,
    currentPage: 1,
    showLoader: false,
...
const ProgramOptionPage: NextPage<ProgramOptionParams> = (props) => {
    const router = useRouter();
    initialPageState.currentPage = props.page;
    const unitOfService = container.get<IUnitOfService>(TYPES.IUnitOfService);
    const [states, dispatch] = useReducer(reducer<ProgramOptionResponseDto>, initialPageState);

    const { formState, handleSubmit, register, setValue, getValues } =
        useForm<ProgramOptionParams>({
            defaultValues: {
    ...
    const deleteProgramOption = async () => {
        dispatch({
            type: InPageActionType.SHOW_LOADER,
            payload: true,
        });

        const response = await unitOfService.ProgramOptionService.delete(states.id);
    ...
    const submitData = async (formData: ProgramOptionParams) => {
        dispatch({
            type: InPageActionType.SHOW_LOADER,
            payload: true,
        });
    ...
    async function actionFunction(p: ProgramOptionParams) {
        const qs = require("qs");
        await fetchProgramOption(p);
        router.push(
            {
    ...
    const fetchProgramOption = async (p?: ProgramOptionParams) => {
        if (!p) {
            p = props;
        }
        ...
        const response = await unitOfService.ProgramOptionService.getAll(p);
        ...
        <>
                        ...
                        <Col md={12} lg={12}>
                            ...
                            >
                                ...
                                <Row>
                                    ...
                                    <Col md={3} className="text-end">
                                        ...
                                        >
                                            <FontAwesomeIcon icon={faPlusCircle} size="1x" /> Add Program
                                        </Button>
                                    </Col>
                                </Row>
                            </Form>
                            ...
                            <div className="tableListItems">
                                ...
                                <Table striped hover className="custom_design_table mb-0">
                                    ...
                                    <tbody>
                                        {states.data &&
                                            states.data?.programOptions?.map((programoption) => {
                                                return (
                                                    <tr key={programoption.id}>
                                                        <td>{programoption.level.name}</td>
                                                        <td>{programoption.name}</td>
                                                        <td>{programoption.timeSchedule}</td>
                                                        <td className="text-center">{unitOfService.CurrencyCodeService.convertToCurrency(programoption.fees)}</td>
                                                        <td className="text-center">
                                                            ...
                                                            >
                                                                <span
                                                                    ...
                                                                    onClick={() => openAddUpdateModal(programoption.id)}
                                                                >
                                                                    <FontAwesomeIcon
                                                                        icon={faPenToSquare}
                                                                        size="1x"
                                                            ...
                                                            >
                                                                <span
                                                                    ...
                                                                    onClick={() => {
                                                                        openDeleteModal(programoption.id);
                                                                    }}
                                                                >
            ...
            {states.showDeleteModal && (
                <ConfirmBox
                    ...
                    onSubmit={deleteProgramOption}
                    bodyText="Are you sure want to delete this program?"
                    noButtonText="Cancel"
...
export default ProgramOptionPage;
