...
import AddOutsidePaymentMethod from "./AddOutsidePaymentMethod";
import { InPageActionType, InPageState, reducer } from "@/reducers/InPageAction";
import { OutSidePaymentMethodListResponseDto } from "@/dtos/OutsidePaymentMethodListResponseDto";

const initialPageState: InPageState<OutSidePaymentMethodListResponseDto> = {
    id: 0,
    currentPage: 1,
    showLoader: false,
...
const OutsidePaymentMethodPage: NextPage<OutsidePaymentMethodListParams> = (props) => {

    const router = useRouter();
    initialPageState.currentPage = props.page;
    const unitOfService = container.get<IUnitOfService>(TYPES.IUnitOfService);
    const [states, dispatch] = useReducer(reducer<OutSidePaymentMethodListResponseDto>, initialPageState);

    const { formState, handleSubmit, register, setValue, getValues, control } =
        useForm<OutsidePaymentMethodListParams>({
            defaultValues: {
                q: props.q,
                page: props.page,
    ...
    const deleteOutsidePaymentMethod = async () => {
        dispatch({
            type: InPageActionType.SHOW_LOADER,
            payload: true,
        });

        const response = await unitOfService.OutsidePaymentMethodService.delete(states.id);
    ...
    const submitData = async (formData: OutsidePaymentMethodListParams) => {
        dispatch({
            type: InPageActionType.SHOW_LOADER,
            payload: true,
        });
    ...
    async function actionFunction(p: OutsidePaymentMethodListParams) {
        const qs = require("qs");
        await fetchOutsidePaymentMethod(p);
        router.push(
            {
    ...
    const fetchOutsidePaymentMethod = async (p?: OutsidePaymentMethodListParams) => {
        if (!p) {
            p = props;
        }
        const response = await unitOfService.OutsidePaymentMethodService.getAll(p);
        ...
        <>
            <div className="parent_list_page">
                <div className="formBlock">
                    <Row>
                        <Col md={12} lg={12}>
                            <p>This list should include all the accepted payment methods that are allowed for transactions outside EDNA. (ex: Cash, check, credit card etc.)</p>
                            <Form
                                method="get"
                                autoComplete="off"
                            ...
                            >
                                ...
                                <Row>
                                    ...
                                    <Col className="text-end">
                                        ...
                                        >
                                            <FontAwesomeIcon icon={faPlusCircle} size="1x" /> Outside EDNA Payments
                                        </Button>
                                    </Col>
                                </Row>
                            </Form>
                            ...
                            <div className="tableListItems">
                                <Table striped hover className="custom_design_table mb-0">
                                    ...
                                    <tbody>
                                        {states.data &&
                                            states.data?.outSidePaymentMethods?.map((outPayMethod) => {
                                                
                                                return (
                                                    <tr key={outPayMethod.id}>
                                                        <td>{outPayMethod.name}</td>
                                                        <td>{outPayMethod.description}</td>
                                                        <td>
                                                            {unitOfService.DateTimeService.convertToLocalDate(
                                                                outPayMethod.createdOn
                                                            )}
                                                        </td>
                                                        <td className="text-center">
                                                            ...
                                                            >
                                                                <span
                                                                    ...
                                                                    onClick={() => openAddUpdateModal(outPayMethod.id)}
                                                                >
                                                                    <FontAwesomeIcon
                                                                        icon={faPenToSquare}
                                                                        size="1x"
                                                            ...
                                                            >
                                                                <span
                                                                    ...
                                                                    onClick={() => {
                                                                        openDeleteModal(outPayMethod.id);
                                                                    }}
                                                                >
            ...
            {states.showDeleteModal && (
                <ConfirmBox
                    ...
                    onSubmit={deleteOutsidePaymentMethod}
...
export default OutsidePaymentMethodPage;
