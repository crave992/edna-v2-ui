...
interface PayNowInvoiceProps extends CommonProps {
  id: number;
  isOpen: boolean;
  onClose: (refreshRequired: boolean) => void;
}

const PayNowInvoice: NextPage<PayNowInvoiceProps> = (props) => {
  ...
  const submitData = async (model: InvoicePaymentModal) => {
    ...
    const response = await unitOfService.InvoiceService.makePayment({
      cvv: model.cvv,
      invoiceId: props.id,
      paymentGatewayId: 1,
