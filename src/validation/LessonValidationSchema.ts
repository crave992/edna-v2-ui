...
const LessonValidationSchema = Yup.object().shape({
    ...
    materialUsed: Yup.string().required("Material is required"),
    ...
    sequentialAssignment: Yup.string().required("Sequential order is required")
});

export default LessonValidationSchema;


export const LessonBulkValidationSchema = Yup.object().shape({
    levelId: Yup.number().test("shouldBeGreaterThanZero", "Level is required", (value) => {
        if (value && value > 0) return true;
        return false;
    }),
    areaId: Yup.number().test("shouldBeGreaterThanZero", "Area is required", (value) => {
        if (value && value > 0) return true;
        return false;
    }),
    topicId: Yup.number().test("shouldBeGreaterThanZero", "Topic is required", (value) => {
        if (value && value > 0) return true;
        return false;
    }),
    document: Yup.mixed()
