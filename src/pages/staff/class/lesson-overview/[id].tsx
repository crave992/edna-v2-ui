...
const LessonPlanningPage: NextPage<ClassLessonOverviewProps> = (props) => {
  ...
  const { handleSubmit, setValue, getValues, control } = useForm<ClassLessonOverviewProps>({
    defaultValues: {
  ...
  const fetchLesson = async (formData: ClassLessonOverviewProps) => {
    if (formData.levelId > 0 && formData.areaId > 0 && formData.topicId > 0 && formData.classId > 0) {
      const response = await unitOfService.LessonService.getByLevelAreaAndTopicAndClass(
        formData.levelId,
        formData.areaId,
        formData.topicId,
        formData.classId,
  ...
  useEffect(() => {
      const storedAreaId = localStorage.getItem('staffLessonPlanAreaId');
      if (storedAreaId) {
  ...
  useEffect(() => {
      const storedTopicId = localStorage.getItem('staffLessonPlanTopicId');
      if (storedTopicId) {
  ...
  return (
    <>
      <Head>
        <title>{`Lesson Planning | ${siteMetadata.title}`}</title>
      </Head>
          ...
          <Row>
            <Col>
              <div className="db_heading_block">
                {classOverview && <h1 className="db_heading">Class: {classOverview.name}</h1>}
              </div>
            </Col>
          </Row>
          <Row>
            <Col className="mb-3">
              <Form method="get" autoComplete="off" onSubmit={handleSubmit(fetchLesson)}>
                <Row>
                  {/* <Col>
                 ...
                 <Col>
                  ...
                  <Col>
                    ...
                    <CustomSelect
                      ...
                      onChange={async (option) => {
                        const selectedId = +(option?.[0] || 0);
                        localStorage.setItem("staffLessonPlanAreaId", (option || '').toString());
                  ...
                  <Col>
                    ...
                    <CustomSelect
                      ...
                      onChange={async (option) => {
                        const selectedId = +(option?.[0] || 0);
                        localStorage.setItem("staffLessonPlanTopicId", (option || '').toString());
                  ...
                  <Col>
                    ...
                    <CustomInput name="q" control={control} placeholder="Name*" onChange={formInputChange} />
                  </Col>
                </Row>
                <Row></Row>
              </Form>
...
export const getServerSideProps: GetServerSideProps<ClassLessonOverviewProps> = async (context) => {
