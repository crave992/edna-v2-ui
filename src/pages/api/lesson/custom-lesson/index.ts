...
const saveRequest = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (token) {
    try {
      ...
      const postUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/Lesson`;
...
const handleCustomLesson = async (req: NextApiRequest, res: NextApiResponse<Response>) => {
  switch (req.method) {
    case 'POST':
...
export default handleCustomLesson;
