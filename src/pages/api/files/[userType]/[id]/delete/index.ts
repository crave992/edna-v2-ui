...
const deleteRequest = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (token) {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/File/DeleteFile/${userType}/${id}`, options);
...
const handleDeleteFile = async (req: NextApiRequest, res: NextApiResponse<Response>) => {
  switch (req.method) {
    case 'DELETE':
