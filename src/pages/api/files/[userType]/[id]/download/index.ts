...
const getRequest = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (token) {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/File/DownloadFile/${userType}/${id}`, options);
...
const handleDownloadFile = async (req: NextApiRequest, res: NextApiResponse<Response>) => {
  switch (req.method) {
    case 'GET':
